...
return true;
	...
	if(!$u)
		{
		echo "Your username '".$wpuser->user_login."' is not a valid MediaWiki username";
	...
	if(!$id) {
		echo"Failed to get ID from name '".$wpuser->user_login."'";
		wp_die("Failed to get ID from name '".$wpuser->user_login."'");
		return true;
	}
	if($id==0) {
		echo("Wikipedia '".$wpuser->user_login."' was not found.");
